...
class Bot
{
    ...
    protected array $updateTypes = ['ReceiveUpdate', 'ReceiveInlineMessage', 'ReceiveQuery', 'GetSelectionItem', 'SearchSelectionItems'];
    ...
    public function getChat(array $data): array
    {
        ...
        $chat = $res['data'] ?? [];
        $this->chat = $chat;
        $this->chat_type = $chat['chat']['chat_type'] ?? null;
        $this->first_name = $chat['chat']['first_name'] ?? null;
        $this->user_name = $chat['chat']['username'] ?? null;
        return $res;
    }
    ...
    public function saveSpamData(): void
    {
        if (!$this->config['spam']) {
            return;
        }
    ...
    private function captureUpdate(): void
    {
        ...
        if ($input) {
            ...
            $this->update = $update;
            $this->set_message_args($update);
        } else {
            $this->update = [];
        }
    }

    private function set_message_args(array $update)
    {
        $this->update_type = $update['update']['type']
            ?? $update['inline_message']['type']
            ?? null;
        ...
        $this->sender_id = $update['update']['new_message']['sender_id']
            ?? $update['inline_message']['sender_id']
            ?? null;

        $this->text = $update['update']['new_message']['text']
        ...
        $this->file_size = $this->update['update']['new_message']['file']['size'] ?? null;
        $this->message_id = $update['update']['new_message']['message_id']
            ?? $update['inline_message']['message_id']
            ?? null;
        return $this;
    }
    ...
    public function run(): void
    {
        if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->getChat(['chat_id' => $this->chat_id]);
            ...
            while (true) {
                try {
                    ...
                    foreach ($updates['data']['updates'] as $update) {
                        ...
                        $this->update = $update1;
                        $this->set_message_args($update1);
                        $this->chat($update['chat_id'] ?? '');
                        $this->getChat(['chat_id' => $update['chat_id']]);
                        $senderId = $update['new_message']['sender_id'] ?? $update['inline_message']['sender_id'];
                        if ($senderId) {
                            if ($this->isUserSpamDetected($senderId)) {
                                continue;
                            }
